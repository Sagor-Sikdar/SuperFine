...
class Tree(object):
	...
	def dfs_traverse(self, visitor):
		...
		for (n, b, l) in self._edges:
			...
			n.dfs_traverse(visitor)             # recursive call
	...
	################################################################################
	#                                                                              #
	# ADDED BY dneves@di.uminho.pt                                                 #
	#                                                                              #
	def iternodes(self, order="preorder"):
		"""
		A generator that allows to iterate over the nodes - (sub)trees - in
		preorder - from root to leaves - or postorder - from leaves to root.

		References:
		[1] This method is almost a verbatim copy of the method `iternodes` found at
			http://hackingtrees.net/chapters/phylogenetics/
		"""
		if order.lower() == "preorder":
			yield self
		for edge in self._edges:
			for node in edge[0].iternodes(order):
				yield node
	...
	edges = property(get_edges, None, None, "List of edges to (sub)trees.")
	leaves = property(get_leaves, None, None, "List of leaves in this (sub)tree.")
	leaves_identifiers = property(get_leaves_identifiers, None, None, "List of identifiers of the leaves in this (sub)tree.")
...
class Leaf(object):
	...
	# ADDED BY dneves@di.uminho.pt
	#
	def iternodes(self, order="ignored"):
		yield self
	#
	############################################################################
	...
	def __repr__(self):
		#want to leave underscores alone
		# - jyang
	...
	leaves_identifiers = property(get_leaves_identifiers, set_leaf_identifier, None, "List of identifiers of the leaves in this subtree.")
...
def parse_tree(input):
	'''
	Parse input as a Newick description of a tree and
	return the tree in a tree data structure.
	'''
