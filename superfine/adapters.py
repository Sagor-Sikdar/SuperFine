...
class MRLAdapter(object):
	...
	[3] A. Stamatakis. RAxML version 8: A tool for phylogenetic analysis and post-analysis of large phylogenies.
		Bioinformatics, 2014.
	[4] Neves, Diogo Telmo and Sobral, Jo�o Lu�s.
	...
	def get_tree(self):
		...
		if supermatrix.number_of_sites:
			...
			f1 = open(filename + ".mr", 'w')  
			f1.write(supermatrix.phylip_format)
			f1.flush()
			f1.close()
			...
			if self.method == "rml":    # RAxML
				...
				########################################################################################################
				#pipe = Popen("java -jar mrp.jar %s.trees %s.aln phylip" % (baseName, baseName), shell = True, stdout = PIPE, stderr = PIPE)
				#(out, err) = pipe.communicate()
